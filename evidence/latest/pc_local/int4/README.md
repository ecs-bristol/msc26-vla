...
| int8 语言-only | 10 | 389.5 | 404.3 | 929.4 | 992.9 |
| int8 全背干 | 10 | 445.1 | 558.1 | 835.9 | 962.8 |
| fp16 | 2 | 144.6 | 189.4 | 1217.9 | 1280.8 |
| int8 语言-only | 2 | 133.3 | 149.2 | 929.4 | 992.9 |
| int8 全背干 | 2 | 142.9 | 148.5 | 835.9 | 962.8 |
| mixed（v4 c4 t8） | 2 | 179.2 | 225.3 | 787.5 | 960.1 |

## 结论

1. 均匀 4-bit PTQ 对 SmolVLA 不成立：全背干 30%、语言-only 10%（单集冒烟）。
2. int8 两种范围都保住并略提升成功率（78-80% vs fp16 72%）。语言-only：权重内存 −23.7%、峰值显存 −22.5%、延迟略降；全背干：权重内存 −31.4%、峰值显存 −24.8%，ns10 延迟略升、ns2 基本持平。
3. num_steps=2 与 int8 正交叠加，形成“少步数 × 8-bit 量化”的组合最优；报告可进一步讨论“语言-only 换速度 vs 全背干换内存”的消融权衡。
4. 混合精度（vision 4bit + connector 4bit + text 8bit）可行：ns2 成功率 76%（vs fp16 72%），权重内存 −35.3%，但延迟升到 179ms（4bit 反量化开销）。说明 text transformer 是 4-bit 敏感点，vision/connector 可降 4-bit。
